...
| unzip | land.sh · reset.sh (courier landing) | `unzip -v \| head -1` |
...
| tar, gzip *(default)* | backup.sh custody tarballs | `tar --version \| head -1` |
...
| contexts/config | planted by `install-configs.sh` from courier staging into the loop user's `~/.serena` | `smoke.sh` serena leg |
