...
1. `./install-configs.sh --check` — clean (no drift, no refusals).
2. `./preflight.sh <member> <cid>` — green (each member being armed) (env walls · cited-input hashes · warmup).
3. `./wall.sh --sweep` holds · `artifacts/instance/tools/audit-consistency.sh` exits 0.
...
5. **serena**: `trusted_project_path_patterns` present in `~/.serena/serena_config.yml`
   (≥1.6 trap, D36) · `./serena-fleet.sh` warm with zero warnings.
...
11. **lanes (when >1, L16)**: each lane's worktree exists and is clean · per-lane locks
    ...
    serena trust globs = the fleet-derived pair for THIS root (serena-fleet.sh prints PASS or the corrective line — L35).
