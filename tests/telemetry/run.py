...
def main() -> int:
    ...
    for line in (HERE / "map.tsv").read_text(encoding="utf-8").splitlines():
        ...
        if test_id in mapping:
            print(f"TELEMETRY INVENTORY FAILURE: duplicate test {test_id}", file=sys.stderr)
            return 2
    ...
    if set(result.outcomes) != test_ids:
        print("TELEMETRY RESULT CHANNEL FAILURE", file=sys.stderr)
        return 2
    print(
        f"TELEMETRY ACCEPTANCE: {len(test_ids)} tests · {len(claim_ids)} claims · "
