...
INVENTORY = {
    ...
    "tests/telemetry/map.tsv",
    "tests/telemetry/run.py",
    "tests/telemetry/test_system_telemetry.py",
    "tests/telemetry/test_telemetry_store.py",
    "tests/telemetry/verify_freeze.py",
}
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(f"TELEMETRY FREEZE FAILURE: {error}", file=sys.stderr)
        return 1
    print(f"TELEMETRY FREEZE VERIFIED — {len(expected)} inputs")
    return 0
