...
def main() -> int:
    ...
    for line in (HERE / "map.tsv").read_text(encoding="utf-8").splitlines():
        ...
        if test_id in mapping:
            print(f"CERTIFICATION INVENTORY FAILURE: duplicate test {test_id}", file=sys.stderr)
            return 2
    ...
    if (
        ...
        print(
            "CERTIFICATION INVENTORY FAILURE: "
            f"tests={len(test_ids)} mapped={len(mapping)} "
            f"claims={len(claim_ids)} covered={len(cited)}",
            file=sys.stderr,
        )
        return 2
    stream = io.StringIO()
    ...
    if set(result.outcomes) != test_ids:
        print("CERTIFICATION RESULT CHANNEL FAILURE", file=sys.stderr)
        return 2
    print(
        f"CERTIFICATION: {len(test_ids)} tests · {len(claim_ids)} claims · "
