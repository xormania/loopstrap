...
mkdir -p "$CR" "$CW/artifacts/reports" "$CW/ops"
cp "$FAMILY/ops/custody-sweep.sh" "$CW/ops/custody-sweep.sh"
printf 'state-v1\n' > "$CR/backlog.md"
printf '{"trace":1}\n' > "$CR/unit-1.jsonl"
( cd "$CW" && bash ops/custody-sweep.sh demo c1 > "$CW/first.log" 2>&1 ); RC=$?
...
printf '{"trace":2}\n' > "$CR/unit-1.jsonl"
( cd "$CW" && bash ops/custody-sweep.sh demo c1 > "$CW/second.log" 2>&1 ); RC=$?
