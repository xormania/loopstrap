...
LW="$FIXROOT/land"; rm -rf "$LW"; mkdir -p "$LW/root/ops" "$LW/stage/ops" "$LW/stage/sub"
mkdir -p "$LW/stage/artifacts/instance/tools"
cp "$FAMILY/ops/land.sh" "$LW/root/ops/land.sh"
cp "$FAMILY/ops/land.sh" "$LW/stage/ops/land.sh"
cp "$FAMILY/artifacts/instance/tools/verify-tree.py" "$LW/stage/artifacts/instance/tools/verify-tree.py"
chmod 0755 "$LW/root/ops/land.sh" "$LW/stage/ops/land.sh" "$LW/stage/artifacts/instance/tools/verify-tree.py"
mkzip(){
...
( cd "$LW/root" && bash ops/land.sh "$LW/courierA.zip" > "$LW/a.log" 2>&1 ); RC=$?
...
[ "$(stat -c %a "$LW/root/ops/land.sh")" = 755 ] && ok "landing: manifested executable mode applied" || no "landing: executable mode drifted"

mkdir -p "$LW/root/xor"; printf 'keep\n' > "$LW/root/xor/scratch.txt"
rm "$LW/stage/sub/beta.txt"; printf 'alpha-v2\n' > "$LW/stage/alpha.txt"
mkzip "$LW/courierB.zip"
( cd "$LW/root" && bash ops/land.sh "$LW/courierB.zip" > "$LW/b.log" 2>&1 ); RC=$?
...
( cd "$LW/root" && bash ops/land.sh "$LW/courierC.zip" >/dev/null 2>&1 )
...
( cd "$LW/root" && bash ops/land.sh "$LW/courierD.zip" > "$LW/d.log" 2>&1 ); RC=$?
...
( cd "$LW/root" && bash ops/land.sh "$LW/courier-extra.zip" > "$LW/extra.log" 2>&1 ); RC=$?
...
PY
( cd "$LW/root" && bash ops/land.sh "$LW/courier-traversal.zip" > "$LW/traversal.log" 2>&1 ); RC=$?
...
PY
( cd "$LW/root" && bash ops/land.sh "$LW/courier-duplicate.zip" > "$LW/duplicate.log" 2>&1 ); RC=$?
...
( cd "$LW/root" && bash ops/land.sh "$LW/courier-tamper.zip" > "$LW/tamper.log" 2>&1 ); RC=$?
...
( cd "$LW/root" && bash ops/land.sh "$LW/courier-noman.zip" > "$LW/noman.log" 2>&1 ); RC=$?
