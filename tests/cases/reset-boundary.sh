...
RESET_VALIDATE_ONLY=1 LOOPSTRAP_ROOT="$HOME" bash "$FAMILY/reset.sh" "$ZIP" > "$RW/home.log" 2>&1; RC=$?
[ "$RC" -ne 0 ] && grep -q "may not equal HOME" "$RW/home.log" \
  && ok "reset: refuses HOME as destructive target" || no "reset: accepted HOME itself"

RESET_VALIDATE_ONLY=1 LOOPSTRAP_ROOT="$RW/outside" bash "$FAMILY/reset.sh" "$ZIP" > "$RW/outside.log" 2>&1; RC=$?
...
RESET_VALIDATE_ONLY=1 LOOPSTRAP_ROOT="$HOME/projects/loopstrap" bash "$FAMILY/reset.sh" "$ZIP" > "$RW/inside.log" 2>&1; RC=$?
...
RESET_VALIDATE_ONLY=1 LOOPSTRAP_ROOT="$HOME/projects/linked-loopstrap" bash "$FAMILY/reset.sh" "$ZIP" > "$RW/link.log" 2>&1; RC=$?
