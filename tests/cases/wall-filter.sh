...
printf 'lsp_math watches the loop for stalls.\n' > "$WF/f2"; "$FAMILY/ops/wall.sh" "$WF/f2" >/dev/null 2>&1; assert_eq "wall R2 fires (member-as-actor)" "$?" "1"
printf 'We use lsp_math to build lsp_math.\n' > "$WF/f3"; "$FAMILY/ops/wall.sh" "$WF/f3" >/dev/null 2>&1; assert_eq "wall R3 fires (product-builds-itself)" "$?" "1"
printf 'Per the lsp_math lexicon the loop must halt.\n' > "$WF/f4"; "$FAMILY/ops/wall.sh" "$WF/f4" >/dev/null 2>&1; assert_eq "wall R4 fires (runtime-doc-as-authority)" "$?" "1"
printf 'The breaker is a component of lsp_math.\n' > "$WF/f5"; "$FAMILY/ops/wall.sh" "$WF/f5" >/dev/null 2>&1; assert_eq "wall R5 fires (dev-inside-product)" "$?" "1"
printf 'The campaign backlog ships in the courier.\n' > "$WF/f6"; "$FAMILY/ops/wall.sh" --lane runtime "$WF/f6" >/dev/null 2>&1; assert_eq "wall R6 fires (dev-vocab-in-contract)" "$?" "1"
...
"$FAMILY/ops/wall.sh" "$WF/c1" >/dev/null 2>&1; assert_eq "wall holds on legit mentions + the wall's own text (FP-12 regression)" "$?" "0"
...
WALL_ALLOW="$WF/allow" "$FAMILY/ops/wall.sh" "$WF/a1" > "$WF/a1.out" 2>&1; assert_eq "wall-allow suppresses ruled exception" "$?" "0"
...
WALL_ALLOW="$WF/allow" "$FAMILY/ops/wall.sh" "$WF/a2" >/dev/null 2>&1; RC=$?
...
WALL_ALLOW="$WF/bad-allow" "$FAMILY/ops/wall.sh" "$WF/a1" >/dev/null 2>&1; RC=$?
assert_eq "wall-allow rejects malformed uncited entries" "$RC" "1"

"$FAMILY/ops/wall.sh" "$WF/does-not-exist" >/dev/null 2>&1; RC=$?
assert_eq "wall refuses missing requested inputs" "$RC" "2"
"$FAMILY/ops/wall.sh" --lane invented "$WF/c1" >/dev/null 2>&1; RC=$?
...
WR="$WF/regex-root"; mkdir -p "$WR/artifacts" "$WR/ops"
cp "$FAMILY/ops/wall.sh" "$WR/ops/wall.sh"
