...
# enforces: install-configs.sh — plant · __ROOT__ render (L35) · manifest · idempotence ·
...
mkdir -p "$IR/artifacts/agent-configs/root" "$IR/artifacts/agent-configs/demo/.claude" "$IR/repos"
cp "$FAMILY/install-configs.sh" "$IR/"
...
( cd "$IR" && bash install-configs.sh > "$IW/i1.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh --check > "$IW/c1.log" 2>&1 ); RC=$?
assert_eq "install: --check clean right after plant (exit 0)" "$RC" "0"
( cd "$IR" && bash install-configs.sh > "$IW/i2.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh --check > "$IW/c2.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh > /dev/null 2>&1 )
( cd "$IR" && bash install-configs.sh --check > "$IW/c3.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh > "$IW/i3.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh --check > "$IW/missing-check.log" 2>&1 ); RC=$?
...
( cd "$IR" && bash install-configs.sh > "$IW/missing-install.log" 2>&1 ); RC=$?
